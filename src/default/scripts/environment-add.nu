...
export def main [
  ...
  let devenv = (open devenv.yaml)

  # TODO: handle the "update" columns not existing...
  $devenv
  | update inputs.environments {flake: false url: "path:src"}
  | update inputs.project {flake: false url: "path:."}
  | update imports (
      $devenv
      | get imports
      | append (
          $environments
          | each {$"environments/($in)"}
        )
      | uniq
      | sort
  )
